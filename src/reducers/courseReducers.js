...
export const courseListReducer = (
  ...
  switch (action.type) {
    ...
    case COURSE_LIST_SORT:
      return {
        ...state,
        sortedCourses: state.courses.filter(
...
export const courseDetailsReducer = (
  state = { course: { reviews: [], instructor: {} } },
  action
) => {
  switch (action.type) {
...
export const enrolledCoursesReducer = (state = { courses: [] }, action) => {
  switch (action.type) {
    ...
    case ENROLLED_COURSE_SORT_BY_RATING:
      return {
        ...state,
        courses: Object.values(state.courses).sort(
          (a, b) => b.course.rating - a.course.rating
        ),
      };
    case ENROLLED_COURSE_SORT_BY_ENROLLED_DATE:
      return {
        ...state,
        courses: state.courses.sort(
