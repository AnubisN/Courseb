...
function CheckoutPage() {
  ...
  useEffect(() => {
    if (!userInfo) {
      ...
      if (success) {
        setSuccessMessage(
          `You have enrolled for this course. You will be redirected to 'My Courses' page.`
        );
  ...
  const onSubmitHandler = (e) => {
    e.preventDefault();
    let similarCourse = enrolledCourses.courses.filter(
      (x) => x.course._id === courses._id
    );
        ...
        let config = {
          ...
          eventHandler: {
            onSuccess(payload) {
              ...
              setLoading(true);
              axios
                .post("/api/khaltiPaymentSuccess/", data)
                .then((response) => {
                ...
                .catch((error) => {
                  setMessage(
                    "Payment was not verified. Please try again later!"
                  );
                  setLoading(false);
                });
            },
      ...
      <div className={classes.container}>
        <div className={classes.checkout}>
          <div className={classes.checkout__methods}>
            <h2>Checkout</h2>
            <form>
              <label className={classes.checkout__methods__label}>
                Billing Address
              </label>
              <input
                className={classes.checkout__methods__label__input}
                type="text"
                value="Nepal"
              />
              <label className={classes.checkout__methods__label}>
              ...
              <div className={classes.checkout__methods__radio}>
                <input
                  className={classes.checkout__methods__radiolabel__input}
                  onChange={(e) => {
                    setPayment("Esewa");
                    setMessage("");
                  }}
                  ...
                  name="payment"
                />
                <label className={classes.checkout__methods__radiolabel}>
                  Esewa
                </label>
              </div>
              <div className={classes.checkout__methods__radio}>
          ...
          <div className={classes.checkout__summary}>
            <form onSubmit={(e) => onSubmitHandler(e)}>
              ...
              <Button type="primary__body">Proceed</Button>
            </form>
          </div>
        </div>
